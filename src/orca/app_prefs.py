...
class OrcaPrefs(object):
    ...
    def _getOrcaModifierKeysString(self, orcaModifierKeys):
        """Returns a string that represents the Orca modifier keys passed
            in."""
    ...
    def _getValueForKey(self, prefsDict, key):
        ...
        if key in prefsDict:
            if key == "voices":
                ...
                value = self._getBrailleSelectionIndicatorString(
                    prefsDict[key])
                ...
                value = self._getVerbalizePunctuationStyleString(
                    prefsDict[key])
